...
function main (options) {
    ...
    if (options.logPerformance) {
        console.log(`Finished clustering at ${perf_hooks.performance.now()}`);
    }
    ...
    return sqlite.open({filename: options.output, driver: sqlite3.Database}, {Promise}).then(db => Promise.all([
            ...
            for (let x = 0; x < zoomDimension; x++) {
                for (let y = 0; y < zoomDimension; y++) {
                    ...
                    compressedTiles.push(
                        nodeGzip.gzip(VTpbf.fromGeojsonVt({'geojsonLayer': tile}, {version: options.tileSpecVersion, extent: options.extent})).then((compressed) => {
        ...
        return Promise.all(compressedTiles).then(() => Promise.all(statements).then(() => {
            // TODO include stats?
            if (options.logPerformance) {
                console.log(`Finished generating MBTiles at ${perf_hooks.performance.now()}.`);
            }
        }));
    }));
}
